...
type ActionMenuModel struct {
	Active  bool
	Cluster internalaws.EKSCluster
	Actions []Action
	Cursor  int
}

func newActionMenu(c internalaws.EKSCluster) ActionMenuModel {
...
type EKSModel struct {
	...
	showDetail     bool
	detailNodeGroups []internalaws.EKSNodeGroup
	detailLoading  bool
}

func New() *EKSModel {
	return &EKSModel{viewState: vsTable, loading: true}
}
...
func (m *EKSModel) Update(msg tea.Msg, s *shared.SharedState) (shared.TabModel, tea.Cmd) {
	...
	case nodeGroupsLoadedMsg:
		...
		m.detailNodeGroups = msg.nodeGroups
		return m, nil
	}
...
func (m *EKSModel) View(s *shared.SharedState) string {
	...
	if m.loading {
		...
		sections = append(sections, lipgloss.NewStyle().Width(s.Width).Padding(2, 2).Render("No EKS clusters found in this region."))
	} else {
	...
	case m.showDetail:
		if m.cursor < len(m.filtered) {
			overlay = RenderDetail(m.filtered[m.cursor], m.detailNodeGroups, m.detailLoading)
		}
...
func (m *EKSModel) updateTable(msg tea.Msg, s *shared.SharedState) (shared.TabModel, tea.Cmd) {
...
func (m *EKSModel) updateSearch(msg tea.Msg, _ *shared.SharedState) (shared.TabModel, tea.Cmd) {
	keyMsg, ok := msg.(tea.KeyPressMsg)
	if !ok {
		return m, nil
	}
...
func (m *EKSModel) updateActionMenu(msg tea.Msg, s *shared.SharedState) (shared.TabModel, tea.Cmd) {
	...
	case "enter":
		if m.actionMenu.Selected() == "detail" {
			...
			m.detailNodeGroups = nil
			return m, m.loadNodeGroups(s, m.actionMenu.Cluster.Name)
		}
	}
	return m, nil
}

func (m *EKSModel) updateDetail(msg tea.Msg, _ *shared.SharedState) (shared.TabModel, tea.Cmd) {
...
func (m *EKSModel) loadData(s *shared.SharedState) tea.Cmd {
	profile := s.Profile
	region := s.Region
	return func() tea.Msg {
...
func (m *EKSModel) loadNodeGroups(s *shared.SharedState, clusterName string) tea.Cmd {
	...
	return func() tea.Msg {
		...
		return nodeGroupsLoadedMsg{nodeGroups: nodeGroups}
	}
}
