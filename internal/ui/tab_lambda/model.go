...
type functionsLoadedMsg struct {
	functions []internalaws.LambdaFunction
	err       error
}
...
type ActionMenuModel struct {
	Active   bool
	Function internalaws.LambdaFunction
	Actions  []Action
	Cursor   int
}

func newActionMenu(fn internalaws.LambdaFunction) ActionMenuModel {
	return ActionMenuModel{
		Active:   true,
		Function: fn,
		Actions: []Action{
			{Key: "detail", Label: "Function Details"},
		},
		Cursor: 0,
	}
}
...
func (a *ActionMenuModel) Render(width int) string {
	...
	b.WriteString(fmt.Sprintf("  %s (%s)\n", a.Function.Name, a.Function.Runtime))
	b.WriteString("  ─────────────────────────\n")
...
// LambdaModel implements the shared.TabModel interface for the Lambda tab.
type LambdaModel struct {
	viewState  viewState
	loading    bool
	err        error
	functions  []internalaws.LambdaFunction
	filtered   []internalaws.LambdaFunction
	cursor     int
	search     SearchModel
...
func New() *LambdaModel {
	return &LambdaModel{viewState: vsTable, loading: true}
}

func (m *LambdaModel) Init(s *shared.SharedState) tea.Cmd {
	m.loading = true
	m.err = nil
	return m.loadData(s)
}

func (m *LambdaModel) Update(msg tea.Msg, s *shared.SharedState) (shared.TabModel, tea.Cmd) {
	switch msg := msg.(type) {
...
func (m *LambdaModel) View(s *shared.SharedState) string {
	var sections []string
	...
	if m.loading {
		sections = append(sections, lipgloss.NewStyle().Width(s.Width).Padding(2, 2).Render("Loading Lambda Functions..."))
	} else if m.err != nil {
		...
		sections = append(sections, lipgloss.NewStyle().Width(s.Width).Padding(2, 2).Render("No Lambda functions found in this region."))
	} else {
...
func (m *LambdaModel) ShortHelp() string {
	switch m.viewState {
	case vsSearch:
...
func (m *LambdaModel) updateTable(msg tea.Msg, s *shared.SharedState) (shared.TabModel, tea.Cmd) {
...
func (m *LambdaModel) updateSearch(msg tea.Msg, _ *shared.SharedState) (shared.TabModel, tea.Cmd) {
	keyMsg, ok := msg.(tea.KeyPressMsg)
	if !ok {
		return m, nil
	}
...
func (m *LambdaModel) updateActionMenu(msg tea.Msg, _ *shared.SharedState) (shared.TabModel, tea.Cmd) {
	keyMsg, ok := msg.(tea.KeyPressMsg)
	if !ok {
		return m, nil
	}
...
func (m *LambdaModel) updateDetail(msg tea.Msg, _ *shared.SharedState) (shared.TabModel, tea.Cmd) {
...
func (m *LambdaModel) applyFilters() {
	result := m.functions
	if m.search.Query != "" {
		q := strings.ToLower(m.search.Query)
		var filtered []internalaws.LambdaFunction
		for _, fn := range result {
			if strings.Contains(internalaws.LambdaSearchFields(fn), q) {
...
func (m *LambdaModel) loadData(s *shared.SharedState) tea.Cmd {
	profile := s.Profile
	region := s.Region
	return func() tea.Msg {
		...
		functions, err := internalaws.FetchLambdaFunctions(ctx, clients.Lambda)
		if err != nil {
			return functionsLoadedMsg{err: err}
		}
