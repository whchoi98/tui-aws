...
// viewState tracks the CloudFront tab's internal view mode.
type viewState int
...
// distributionsLoadedMsg is returned when distributions are fetched.
type distributionsLoadedMsg struct {
	distributions []internalaws.Distribution
	err           error
}

// Action represents a menu action for a distribution.
type Action struct {
	Key   string
	Label string
}
...
type ActionMenuModel struct {
	Active       bool
	Distribution internalaws.Distribution
	Actions      []Action
	Cursor       int
}

func newActionMenu(d internalaws.Distribution) ActionMenuModel {
	return ActionMenuModel{
		Active:       true,
		Distribution: d,
		Actions: []Action{
			{Key: "detail", Label: "Distribution Details"},
		},
		Cursor: 0,
	}
}
...
func (a *ActionMenuModel) Render(width int) string {
	...
	b.WriteString(fmt.Sprintf("  %s (%s)\n", a.Distribution.ID, a.Distribution.DomainName))
	b.WriteString("  ─────────────────────────\n")
...
// CloudFrontModel implements the shared.TabModel interface for the CloudFront tab.
type CloudFrontModel struct {
	...
	distributions []internalaws.Distribution
	filtered      []internalaws.Distribution
	cursor        int
...
// New creates a new CloudFrontModel.
func New() *CloudFrontModel {
...
func (m *CloudFrontModel) Init(s *shared.SharedState) tea.Cmd {
	m.loading = true
	m.err = nil
	return m.loadDistributions(s)
}

func (m *CloudFrontModel) Update(msg tea.Msg, s *shared.SharedState) (shared.TabModel, tea.Cmd) {
	switch msg := msg.(type) {
	...
	case distributionsLoadedMsg:
		...
		m.distributions = msg.distributions
		m.applyFilters()
		return m, nil
	}
...
func (m *CloudFrontModel) View(s *shared.SharedState) string {
	var sections []string
	...
	if m.loading {
		sections = append(sections, lipgloss.NewStyle().Width(s.Width).Padding(2, 2).Render("Loading CloudFront Distributions..."))
	} else if m.err != nil {
		...
		sections = append(sections, lipgloss.NewStyle().Width(s.Width).Padding(2, 2).Render("No CloudFront Distributions found."))
	} else {
	...
	case m.showDetail:
		if m.cursor < len(m.filtered) {
			overlay = RenderDistributionDetail(m.filtered[m.cursor])
		}
...
func (m *CloudFrontModel) ShortHelp() string {
	switch m.viewState {
	case vsSearch:
...
func (m *CloudFrontModel) updateTable(msg tea.Msg, s *shared.SharedState) (shared.TabModel, tea.Cmd) {
	...
	case "R":
		m.loading = true
		m.err = nil
		return m, m.loadDistributions(s)
	}

	return m, nil
}

func (m *CloudFrontModel) updateSearch(msg tea.Msg, s *shared.SharedState) (shared.TabModel, tea.Cmd) {
	keyMsg, ok := msg.(tea.KeyPressMsg)
	if !ok {
		return m, nil
	}
...
func (m *CloudFrontModel) updateActionMenu(msg tea.Msg, s *shared.SharedState) (shared.TabModel, tea.Cmd) {
	keyMsg, ok := msg.(tea.KeyPressMsg)
	if !ok {
		return m, nil
	}
...
func (m *CloudFrontModel) updateDetail(msg tea.Msg, _ *shared.SharedState) (shared.TabModel, tea.Cmd) {
...
func (m *CloudFrontModel) applyFilters() {
	result := m.distributions

	if m.search.Query != "" {
		q := strings.ToLower(m.search.Query)
		var filtered []internalaws.Distribution
		for _, d := range result {
			if strings.Contains(strings.ToLower(d.ID), q) ||
				strings.Contains(strings.ToLower(d.DomainName), q) ||
				strings.Contains(strings.ToLower(d.Comment), q) ||
				strings.Contains(strings.ToLower(strings.Join(d.Aliases, " ")), q) ||
				strings.Contains(strings.ToLower(strings.Join(d.Origins, " ")), q) {
...
func (m *CloudFrontModel) loadDistributions(s *shared.SharedState) tea.Cmd {
	profile := s.Profile
	region := s.Region
	return func() tea.Msg {
		...
		if err != nil {
			return distributionsLoadedMsg{err: err}
		}
		distributions, err := internalaws.FetchDistributions(ctx, clients.CF)
		if err != nil {
			return distributionsLoadedMsg{err: err}
		}
		return distributionsLoadedMsg{distributions: distributions}
	}
}
